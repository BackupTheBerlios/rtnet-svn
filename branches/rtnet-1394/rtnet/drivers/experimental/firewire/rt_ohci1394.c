 ...
 *
 * Zhang Yuchen <y.zhang-4@student.utwente.nl>
 *    Ported to rtai (RTnet). 
 *
 */
...
#ifdef CONFIG_TASKHANDOVER_DEBUG
#define TH_FIFO 0
#define BH_FIFO 1
#endif
...
#ifdef CONFIG_TASKHANDOVER_DEBUG
static RTIME th_time;
static RTIME bh_time;

static inline void read_tophalf_time()
{
	th_time = rt_get_cpu_time_ns();
	rtf_put(TH_FIFO,&th_time,sizeof(RTIME));
}

static inline void read_bottomhalf_time()
{
	
	bh_time = rt_get_cpu_time_ns();
	rtf_put(BH_FIFO,&bh_time,sizeof(RTIME));
}
#endif		
				       ...
				       quadlet_t tx_event)
{
	...
	rtos_spin_lock(&ohci->iso_tasklet_list_lock);
	...
	rtos_spin_unlock(&ohci->iso_tasklet_list_lock);
                             ...
                             struct pt_regs *regs_are_unused)
{
	...
	int phyid = -1, isroot = 0, to_trigger_bh = 0;
	unsigned long flags;
...
selfid_not_valid:
	...
	if(to_trigger_bh)
	{
		//PRINT(KERN_INFO,ohci->id, "trigger bottomhalf now\n");	
		rtos_trigger_bh();
	}	
	
	rtos_irq_enable(ohci->dev->irq);	      
	return;
}
...
#define PCI_DEVICE_ID_NVIDIA_NFORCE2_FW 0x006e
	...
	if (rtos_irq_request(dev->irq, ohci_irq_handler, ohci))
		FAIL(-ENOMEM, "Failed to allocate shared interrupt %d", dev->irq);
	else{
		PRINT(KERN_INFO,ohci->id,"irq: %d requested",dev->irq);
	}
	...
	PRINT(KERN_INFO,ohci->id,"probe done\n");
				  ...
				  struct ohci1394_iso_tasklet *tasklet)
{
	...
	rtos_spin_lock_irqsave(&ohci->iso_tasklet_list_lock, flags);
	...
	rtos_spin_unlock_irqrestore(&ohci->iso_tasklet_list_lock, flags);
...
static void ohci1394_cleanup (void)
{
	...
	rtf_destroy(BH_FIFO);
#endif
  //rtos_tasklet_scheduler_exit(tasklet_scheduler);
	pci_unregister_driver(&ohci1394_pci_driver);
	return;
}

static int ohci1394_init(void)
{
#ifdef CONFIG_TASKHANDOVER_DEBUG	
	// to start the rtai timer for latency measuring	
	if(1){
		rt_set_oneshot_mode();
		start_rt_timer(0);
	} 
	...
	// to create the fifo for storing the "time"
	if(rtf_create(TH_FIFO,80000))
	{
		rtos_print(KERN_INFO "fail to create the TH_FIFO!\n");
	}
	
	if(rtf_create(BH_FIFO,80000))
	{
		rtos_print(KERN_INFO "fail to create the BH_FIFO!\n");
	}
#endif
	
	return pci_module_init(&ohci1394_pci_driver);
  //rtos_print("pci_module_init\n");
  //if ((err = rtos_tasklet_scheduler_init(TASKLET_SCHED_PRI,tasklet_scheduler)) !=0)
   // PRINT_G(KERN_ERR,"tasklet_scheduler not initialized!\n");
   //return err;
}
