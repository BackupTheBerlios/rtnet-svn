...
void t21142_timer(unsigned long data)
{
	...
	if (tulip_debug > 2)
		/*RTnet*/rt_printk(KERN_INFO"%s: 21143 negotiation status %8.8x, %s.\n",
	...
	if (tulip_media_cap[rtdev->if_port] & MediaIsMII) {
		...
		if (tulip_debug > 1)
			/*RTnet*/rt_printk(KERN_INFO"%s: Using NWay-set %s media, csr12 %8.8x.\n",
		...
		if (csr12 & 2) {	/* No 100mbps link beat, revert to 10mbps. */
			if (tulip_debug > 1)
				/*RTnet*/rt_printk(KERN_INFO"%s: No 21143 100baseTx link beat, %8.8x, "
					   "trying NWay.\n", rtdev->name, csr12);
		...
		if (tulip_debug > 1)
			/*RTnet*/rt_printk(KERN_INFO"%s: 21143 negotiation failed, status %8.8x.\n",
				   rtdev->name, csr12);
		...
		if (tulip_debug > 1)
			/*RTnet*/rt_printk(KERN_INFO"%s: Testing new 21143 media %s.\n",
...
void t21142_start_nway(/*RTnet*/struct rtnet_device *rtdev)
{
	...
	if (tulip_debug > 1)
		/*RTnet*/rt_printk(KERN_DEBUG "%s: Restarting 21143 autonegotiation, csr14=%8.8x.\n",
			   rtdev->name, csr14);
...
void t21142_lnk_change(/*RTnet*/struct rtnet_device *rtdev, int csr5)
{
	...
	if (tulip_debug > 1)
		/*RTnet*/rt_printk(KERN_INFO"%s: 21143 link status interrupt %8.8x, CSR5 %x, "
	...
	if (tp->nway  &&  !tp->nwayset  &&  (csr12 & 0x7000) == 0x5000) {
		...
		if (tulip_debug > 1) {
			if (tp->nwayset)
				...
				/*RTnet*/rt_printk(KERN_INFO "%s: Autonegotiation failed, using %s,"
		...
		if (tulip_debug > 2)
			/*RTnet*/rt_printk(KERN_DEBUG "%s:  Restarting Tx and Rx, CSR5 is %8.8x.\n",
				   rtdev->name, inl(ioaddr + CSR5));
#endif
		tulip_start_rxtx(tp);
		if (tulip_debug > 2)
			/*RTnet*/rt_printk(KERN_DEBUG "%s:  Setting CSR6 %8.8x/%x CSR12 %8.8x.\n",
		...
		if (tulip_debug > 1)
			/*RTnet*/rt_printk(KERN_INFO"%s: 21143 %s link beat %s.\n",
				   rtdev->name, medianame[rtdev->if_port],
				   (csr12 & 2) ? "failed" : "good");
		...
		if ((csr12 & 4) == 0)
			/*RTnet*/rt_printk(KERN_INFO"%s: 21143 10baseT link beat good.\n",
				   rtdev->name);
	} else if (!(csr12 & 4)) {		/* 10mbps link beat good. */
		if (tulip_debug)
			/*RTnet*/rt_printk(KERN_INFO"%s: 21143 10mbps sensed media.\n",
				   rtdev->name);
		rtdev->if_port = 0;
	} else if (tp->nwayset) {
		if (tulip_debug)
			/*RTnet*/rt_printk(KERN_INFO"%s: 21143 using NWay-set %s, csr6 %8.8x.\n",
				   rtdev->name, medianame[rtdev->if_port], tp->csr6);
	} else {		/* 100mbps link beat good. */
		if (tulip_debug)
			/*RTnet*/rt_printk(KERN_INFO"%s: 21143 100baseTx sensed media.\n",
				   rtdev->name);
		rtdev->if_port = 3;
